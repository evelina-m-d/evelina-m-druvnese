...
def pasutijums(piegadatajs, detala): #ļauj pasūtīt detaļas un ievieto datus par tām failā
    while True:
        ...
        while True:
            try:
                with open('pasutijumi.json', 'r', encoding='utf8') as file: #cenšas ielasīt pasūtijumu faila saturu
                    pasutijumi = json.load(file) 
                break
            except FileNotFoundError:   #ja fails neeksistē, izveido to un noformatē ar tukšu sarakstu, lai to var izmantot turpmāk
                tukss_fails = []
                with open('pasutijumi.json', 'a', encoding='utf8') as file:
                    json.dump(tukss_fails, file)
    ...
    while True:
        ...
        while True:
            try:
                ...
                tukss_fails = []
                with open('noliktava.json', 'a', encoding='utf8') as file:
                    json.dump(tukss_fails, file)
